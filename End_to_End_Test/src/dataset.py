...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        all_in_out = torch.empty(size=(n_views, 3, 256, 256))
        mask_out = torch.empty(size=(n_views, 3, 256, 256))
        gt_out = torch.empty(size=(n_views, 3, 256, 256))
        ...
        if self.training == 1:
            for ind, i in enumerate(self.sample_views):
                ...
                # img_in = np.array(imread(self.data[int(index / self.N_samples)].replace('GT', 'in')
                # + '/{0:1d}'.format(

...
class InpaintingModel(BaseModel):
    ...
    def process(self, images, masks, GTs, n_views):
        ...
        GTs = GTs.view(GTs.shape[0], n_views*6, 256, 256)
        masks = masks.view(masks.shape[0], n_views * 6, 256, 256)

        dis_input_real = GTs
        dis_input_fake = outputs.detach()

        for i in range(0, n_views):
            dis_real, _ = self.discriminator(dis_input_real[:, (6*i): (6*i) + 6, :, :])
            dis_fake, _ = self.discriminator(dis_input_fake[:, (6*i): (6*i) + 6, :, :])
            ...
            gen_fake, _ = self.discriminator(gen_input_fake[:, (6*i): (6*i) + 6, :, :])
            gen_gan_loss = self.adversarial_loss(gen_fake, True, False) * self.config.INPAINT_ADV_LOSS_WEIGHT
            gen_loss += gen_gan_loss/n_views

            gen_content_loss = self.perceptual_loss(outputs[:, (5 * i) + i: ((5 * i) + i) + 3, :, :],
                                                    GTs[:, (5 * i) + i: ((5 * i) + i) + 3, :, :])
            gen_content_loss = gen_content_loss * self.config.CONTENT_LOSS_WEIGHT
            gen_loss += gen_content_loss / n_views

            gen_style_loss = self.style_loss(
                outputs[:, (5 * i) + i: ((5 * i) + i) + 3, :, :] * masks[:, (5 * i) + i: ((5 * i) + i) + 3, :, :],
                GTs[:, (5 * i) + i: ((5 * i) + i) + 3, :, :] * masks[:, (5 * i) + i: ((5 * i) + i) + 3, :, :])

...
def read_image(image_file_list):
    ...
    for output_file in image_file_list:
        ...
        im = imageio.imread(output_file, format='png')
        im_gt = (im > 0.0001)
...
def to_dep(dist_maps, rgb_imgs):

    point_list_canonical = []
    color_intensities = []

    view_id = 0
    for dist_map, rgb_img in zip(dist_maps, rgb_imgs):
    ...
    return {'pc': point_list_canonical, 'color': color_intensities}
...
class PC_from_DEP(object):
    def __init__(self, metadata_dir, camera_path, view_ids, with_normal=True):
        ...
        rgb_img_dir = [os.path.join(metadata_dir, 'color_{0:03d}.png'.format(view_id + 1)) for view_id in view_ids]

        self._dist_maps = read_image_depth(dist_map_dir)
        self._rgb_imgs = read_image(rgb_img_dir)
        self._cam_K = np.loadtxt(os.path.join(camera_path, 'cam_K/cam_K.txt'))
        self._point_clouds = to_dep(self._dist_maps, self._rgb_imgs)
    ...
    def draw3D(self, output, views):
        ...
        for i in range(views):
            ...
            rgb = np.array(inlier_cloud.colors)
            ...
            if i == 0 :
                ...
                colors_all = np.concatenate((colors_all, rgb), axis=0)

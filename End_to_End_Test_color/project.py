...
def get_depth_color(point_cloud, cam_K, cam_RTs, dodecahedron, output_file_path):
    ...
    for index, cam_RT, cam_pos in zip(indexs, cam_RTs, dodecahedron):
        ...
        rgb_imgs = np.zeros([256, 256, 3], dtype=np.uint8)
        ...
        rgb_imgs[u, v, 0] = np.uint8(colors_in[:, 0] * 255)
        rgb_imgs[u, v, 1] = np.uint8(colors_in[:, 1] * 255)
        rgb_imgs[u, v, 2] = np.uint8(colors_in[:, 2] * 255)

        im = Image.fromarray(rgb_imgs)
        im.save(output_file_path + "color_{0:03d}.png".format(index + 5))
